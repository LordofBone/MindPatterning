...
def gatherer(data_type, file_no, time):
    ...
    for x in range(file_no):
        ...
        data_reader("relaxed", "input_{0}_data_relaxed_{1}.csv".format(data_type, x), time)

        print("Now excite your mind...\n")

        data_reader("non-relaxed", "input_{0}_data_nonrelaxed_{1}.csv".format(data_type, x), time)
